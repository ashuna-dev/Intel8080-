...
from registers import RegID, DRegID, flags as fl
...
class ACIInstruction(Instruction):
    ...
    def __call__(self, *args, **kwargs):
        ...
        flags = self._cpu.registers.increment(RegID.A, immediate + self._cpu.condition_flags.cy)
...
class ADDInstruction(Instruction):
    ...
    def __call__(self, *args, **kwargs):
        ...
        flags = self._cpu.registers.increment(RegID.A, addend)
        self._update_flags(flags)
...
class ConditionalCallInstruction(CALLInstruction):
    def _check_condition(self):
        raise NotImplementedError("Subclasses must implement _check_condition")

    def __call__(self, *args, **kwargs):
        if self._check_condition():
            super(ConditionalCallInstruction, self).__call__(*args, **kwargs)

class CCInstruction(ConditionalCallInstruction):
    def _check_condition(self):
        return self._cpu.condition_flags.cy

    def __str__(self):
        return 'CC'

class CMInstruction(ConditionalCallInstruction):
    def _check_condition(self):
        return self._cpu.condition_flags.s
